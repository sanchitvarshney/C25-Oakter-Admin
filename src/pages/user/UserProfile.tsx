...
const UserProfile = () => {
  ...
  const {
    ...
    updateUserVerificationLoading,
    updateUserLoginStatusLoading,
    loading,
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      setLoginStatus(userProfile?.login_status || "");
      setPasswordChange(userProfile?.askChangePassword === "Y");
      setModify2FactorAuthStatus(userProfile?.twoStep === "ON");
    }
  }, [userProfile]);
  ...
  const handleSubmit2 = () => {
    ...
    const payload: any = {
      ...
      askPasswordChange: askPasswordChange ? "Y" : "N",
    };
    ...
    dispatch(changeUserPassword(payload) as any).then((res: any) => {
      ...
      if (res?.payload?.data?.success) {
        ...
        showToast(res?.payload?.data?.message|| "Error resetting password", "error");
      }
    });
  };
  ...
  const handleRequirePasswordChange = (status: any) => {
    const payload:any = {
      ...
      askPasswordChange: status === true  ? 1 : 0,
    };
  ...
  const handleModify2FactorAuth = (status: any) => {
   ...
   dispatch(changeUserPassword(payload)).then((res: any) => {
      if (res?.payload?.data?.success) {
        ...
        showToast("2FA reset successful", "success");
        dispatch(getUserProfile(params.id || ""));
        setPassword("");
        setConfirmPassword("");
      } else {
  ...
  return (
    ...
    {loading && <FullPageLoading/>}
      ...
      <div>
        ...
        >
          ...
          <div className="h-[50px] flex items-center justify-end px-[20px] border-t">
            <Button
              disabled={cahngeUserPasswordLoading}
              onClick={() => setRequiredChangePass(false)}
            >
              Close
            </Button>
            <Button
              disabled={cahngeUserPasswordLoading}
              onClick={() => handleRequirePasswordChange(passwordChange)}
            >
      ...
      <div>
        ...
        >
          ...
          <div className="h-[50px] flex items-center justify-end px-[20px] border-t">
            <Button
              disabled={cahngeUserPasswordLoading}
              onClick={() => setModify2FactorAuth(false)}
            >
              Close
            </Button>
            <Button
        ...
        >
          ...
          <div className="p-[30px] relative flex flex-col gap-[30px]">
            ...
            <div className="flex items-center justify-end gap-[10px]">
              ...
              <LoadingButton
                loading={activateUserLoading || suspendUserLoading}
                disabled={
                  activateUserLoading ||
                  suspendUserLoading ||
      ...
      <Modal
        open={updateLoginStatus}
        onClose={setUpdateLoginStatus}
        aria-labelledby="modal-modal-title"
        aria-describedby="modal-modal-description"
      >
        <Box
        ...
        >
          ...
          <div className="p-[30px] relative flex flex-col gap-[30px]">
            <div className="space-y-5">
              <Typography sx={{ marginRight: 2 }}>Login Status</Typography>
              <FormControlLabel
                value="1"
                control={<Radio />}
                label="Active"
                checked={loginStatus === "1"}
                onChange={() => setLoginStatus("1")}
              />
              <FormControlLabel
                value="0"
                control={<Radio />}
                label="Inactive"
                checked={loginStatus === "0"}
                onChange={() => setLoginStatus("0")}
              />
            </div>
            <div className="flex items-center justify-end gap-[10px]">
              <Button
                disabled={updateUserLoginStatusLoading}
                onClick={() => {
              ...
              <Button
                ...
                variant="contained"
              >
                {updateUserLoginStatusLoading ? "Updating..." : "Continue"}
              </Button>
            </div>
            <div className="absolute bottom-0 left-0 right-0 bg-white h-[20-px]">
              {updateUserLoginStatusLoading && <LinearProgress />}
            </div>
          </div>
        </Box>
      </Modal>
          ...
          <div className="p-[30px] relative flex flex-col gap-[30px]">
            <div className="space-y-5">
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                ...
                <Select
                  ...
                  disabled={updateUserVerificationLoading}
                >
            ...
            <div className="flex items-center justify-end gap-[10px]">
              ...
              <Button
                ...
                variant="contained"
              >
                {updateUserVerificationLoading ? "Updating..." : "Continue"}
              </Button>
            </div>
            <div className="absolute bottom-0 left-0 right-0 bg-white h-[20-px]">
              {updateUserVerificationLoading && <LinearProgress />}
            </div>
          </div>
        </Box>
      </Modal>
          ...
          <div className="w-full rounded-sm shadow shadow-stone-400">
            ...
            >
              <List>
                ...
                <ListItemButton
                  disabled={!userProfile}
                  onClick={() => setUpdateLoginStatus(true)}
                >
                  <Typography fontSize={15} fontWeight={500} variant="inherit">
                    UPDATE STATUS
                  </Typography>
